...
DMS has a rich plugin ecosystem (75+ community plugins) with a Nix flake registry that enables declarative management. cairn deeply integrates DMS but currently uses zero community plugins. The registry provides a clean module that hooks into `programs.dank-material-shell.plugins`, and cairn is uniquely positioned to auto-enable relevant plugins based on existing module flags (formFactor, services.ai, modules.virtualisation, etc.) — the same conditional wiring pattern used for MCP servers.
...
- Wire conditional plugin enablement based on existing Cairn module/hardware flags:
...
- Explicitly disable `nixMonitor` (cairn-monitor already provides this, customized for Cairn)
- All other registry plugins remain available but disabled — users opt in via their downstream config

## Capabilities

### New Capabilities
- `dms-plugins`: Declarative DMS plugin management via the community registry, with conditional auto-enablement based on Cairn module flags
