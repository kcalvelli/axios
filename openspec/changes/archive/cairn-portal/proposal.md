# Proposal: cairn Portal PWA

## Summary

Create an cairn Portal - a service discovery dashboard that lists all available cairn services on the user's tailnet, providing a unified entry point for the cairn ecosystem.

## Motivation

### Problem Statement

As cairn expands its service ecosystem, users face discovery challenges:
...
Create an cairn Portal - a lightweight web dashboard that:

- Lists all configured cairn services
...
1. **Service Cards**: Visual tiles for each cairn service
...
5. **Auto-Discovery**: Reads from cairn configuration
...
```nix
services.portal = {
  enable = lib.mkEnableOption "cairn Portal service discovery dashboard";

  port = lib.mkOption {
    type = lib.types.port;
    default = 8082;
    description = "Local port for cairn Portal";
  };
  ...
  # Auto-populated from other cairn services
  # Users don't configure this directly
  services = lib.mkOption {
    type = lib.types.attrsOf (lib.types.submodule { ... });
    internal = true;
    description = "Auto-discovered cairn services";
  };
};
```
...
The portal automatically discovers configured cairn services:
...
home.file.".local/share/cairn-portal/index.html".text = ''
  <!DOCTYPE html>
  <html>
  <head>
    <title>cairn Portal</title>
...
```nix
xdg.desktopEntries.cairn-portal = {
  name = "Cairn Portal";
  comment = "Service discovery dashboard for cairn ecosystem";
...
- File: `home/resources/pwa-icons/cairn-portal.png`
...
| **cairn Portal** | **8082** | **8445** |
| cairn-calendar | 8083 | 8446 |
...
5. Access all cairn services from one place
...
- **Requires**: At least one cairn service with Tailscale serve
- **Optional**: All other cairn services (auto-discovered)
...
- Custom service registration (non-cairn services)
...
**Rejected**: Overkill for cairn, doesn't auto-discover cairn services.
...
**Rejected**: Poor mobile UX, no status visibility, cairn should be cohesive.

## References

- cairn-mail PWA: `home/pim/default.nix`
