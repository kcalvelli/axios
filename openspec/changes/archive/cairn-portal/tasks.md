# Tasks: cairn Portal PWA

## Overview

Create an cairn Portal - a service discovery dashboard that lists all available cairn services on the user's tailnet.
...
### Task 2.2: Auto-Discover cairn Services
- [ ] Check for cairn-mail configuration
...
in
{
  config = lib.mkIf (portalCfg.enable or false) {
    ...
    home.file.".local/share/cairn-portal/index.html".text = html;

    # Copy service icons
    home.file.".local/share/cairn-portal/icons/cairn-mail.png".source =
      ../resources/pwa-icons/cairn-mail.png;
...
- [ ] Serve from `~/.local/share/cairn-portal/`
- [ ] Listen on configured port

```nix
# Simple Python server approach
systemd.user.services.cairn-portal = {
  Unit = {
    Description = "cairn Portal HTTP server";
    After = [ "network.target" ];
  };
  Service = {
    Type = "simple";
    WorkingDirectory = "%h/.local/share/cairn-portal";
...
- [ ] Design icon: NixOS snowflake + cairn colors + grid/dashboard center
- [ ] Create 128x128 PNG
- [ ] Save to `home/resources/pwa-icons/cairn-portal.png`
...
```nix
xdg.desktopEntries.cairn-portal = lib.mkIf (portalCfg.pwa.enable or false) {
  name = "Cairn Portal";
...
home.file.".local/share/icons/hicolor/128x128/apps/cairn-portal.png" = {
  source = ../resources/pwa-icons/cairn-portal.png;
};
```
...
| `home/resources/pwa-icons/cairn-portal.png` | Portal icon |
