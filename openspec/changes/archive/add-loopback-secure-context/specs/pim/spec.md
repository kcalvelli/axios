...
- **Then**: Desktop entry is created for "Cairn Mail"
- **And**: URL is `https://cairn-mail.${tailnetDomain}/` (same as client)
- **And**: Icon is cairn-mail icon
- **And**: StartupWMClass is `brave-cairn-mail.${tailnetDomain}__-Default`
...
The `/etc/hosts` entry mapping `cairn-mail.local` to `127.0.0.1` is removed from the PIM module.
...
The `--test-type --unsafely-treat-insecure-origin-as-secure=http://cairn-mail.local:<port>` Chromium flags are removed from the PWA desktop entry.
