...
- [x] 1.1 Remove `cairn.user` options (`name`, `fullName`, `email`) and all associated config logic
- [x] 1.2 Define `cairn.users.<name>` as `attrsOf submodule` with options: `fullName` (str), `email` (str, default ""), `isAdmin` (bool, default false), `homeProfile` (enum "workstation"/"laptop"/"minimal"/null, default null), `extraGroups` (list of str)
- [x] 1.3 Implement per-user account creation: for each user in `cairn.users`, create `users.users.<name>` with `isNormalUser = true`, `description = fullName`, and computed groups (module-auto-groups with `wheel` only for `isAdmin = true`, plus per-user `extraGroups`)
- [x] 1.4 Implement per-user XDG directory creation via `systemd.tmpfiles.rules` for all users in `cairn.users`
- [x] 1.5 Implement per-user home-manager wiring: set `home-manager.users.<name>.cairn.user.email` for each user
- [x] 1.6 Set `nix.settings.trusted-users` to list of usernames where `isAdmin = true`
- [x] 1.7 Keep `cairn.users.autoGroups`, `cairn.users.extraGroups`, and `cairn.users.defaultExtraGroups` options (existing group computation logic stays)
...
- [x] 2.3 Implement per-user home profile wiring: for each user in `cairn.users`, resolve their `homeProfile` (fall back to host `homeProfile` if null) and add appropriate profile modules to `home-manager.users.<name>.imports`
...
- [x] 4.1 Create new `user.nix.template` for per-user files using `cairn.users.<name>` format with `fullName`, `email`, `isAdmin`, and `homeProfile` options
...
- [x] 5.1 Update any example configurations (minimal-flake, multi-host) referenced in CI to use the new canonical structure with `cairn.users`, `configDir`, and `users` list
...
- [x] 6.2 Update `CLAUDE.md` and `.claude/project.md` with: new canonical config structure, `cairn.users` pattern, `configDir` + `users` list in host configs, migration guide from old format
- [x] 6.3 Add downstream migration notes: step-by-step guide for converting `cairn.user` → `cairn.users`, `userModulePath` → `users` list, `user.nix` → `users/<name>.nix`
