...
Yesterday's install of a new host with multiple users exposed critical gaps in the Cairn new-host experience. We pushed 6 hotfix commits to master — missing DMS placeholders, broken NVIDIA kernel module, missing AMD VA-API config, deprecated xorg references. The multi-user setup required a 196-line hand-rolled `users/mini.nix` that bypasses the `cairn.user` framework entirely, with 3 copies of the same PWA config and manual group filtering.

The root cause is twofold: (1) Cairn has no multi-user model — `cairn.user` supports exactly one user, forcing multi-user hosts to work around the framework; (2) Cairn doesn't prescribe how downstream configs should be structured — every host evolves its own wiring pattern, and `userModulePath` is a stringly-typed hack that breaks the moment you need more than one user.

This is a **breaking change** that establishes Cairn as a convention-over-configuration framework with a prescribed downstream config structure.

## What Changes

### Canonical Downstream Config Structure
- **BREAKING**: Cairn prescribes the following structure for all downstream configs:
  ```
  ~/.config/nixos_config/
  ├── flake.nix              # Uses cairn-prescribed pattern
  ├── users/                 # One file per user, referenced by name
  │   ├── keith.nix          # cairn.users.keith = { ... }
  │   └── traci.nix
...
- **BREAKING**: Replace `cairn.user` (singular) with `cairn.users.<name>` (attrset of user submodules)
...
- Host configs declare `users = [ "keith" "traci" ]` — Cairn imports the corresponding user files
...
- `system`: User management rebuilt around `cairn.users.<name>` (removing `cairn.user`); `mkSystem` gains `configDir` + `users` list resolution; init script generates canonical structure with multi-user support; DMS placeholder robustness

## Impact

- **BREAKING — modules/users.nix**: Complete rewrite — `cairn.user` removed, `cairn.users.<name>` submodule added
...
- **BREAKING — Downstream configs**: Must restructure to canonical layout — `user.nix` → `users/<name>.nix`, `userModulePath` → `users` list, `cairn.user` → `cairn.users`
