...
This change makes Cairn opinionated about downstream structure and replaces the singular user model with a multi-user framework.

## Goals / Non-Goals

**Goals:**
- Prescribe a canonical downstream config structure that Cairn enforces
- Multi-user support via `cairn.users.<name>` attrset with admin/non-admin roles
...
**Non-Goals:**
- Backward compatibility with `cairn.user` or `userModulePath` — this is a clean break
...
**Decision**: Cairn prescribes this structure:
```
<config-dir>/
├── flake.nix
├── users/
...
### D2: `cairn.users.<name>` as attrset of submodules (replacing `cairn.user`)

**Decision**: Remove `cairn.user` entirely. Replace with `cairn.users` using `lib.types.attrsOf (lib.types.submodule { ... })`. Each user gets: `fullName` (str), `email` (str, default ""), `isAdmin` (bool, default false), `homeProfile` (enum or null, default null), `extraGroups` (list of str).

**Why**: The singular `cairn.user` was the root cause of the multi-user workaround pattern. An attrset of submodules is idiomatic NixOS (same pattern as `users.users.<name>`) and naturally supports any number of users.

**Alternative considered**: Keep `cairn.user` with a backward compat shim — rejected per user decision to make this a breaking change and force downstream conformance.
...
**Why**: This eliminates all stringly-typed path wiring from host configs. Users are referenced by name (the same name as their filename and their `cairn.users` key). The resolution logic lives in one place (`lib/default.nix`).
