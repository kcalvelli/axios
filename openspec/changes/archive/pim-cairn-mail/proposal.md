# Proposal: Replace PIM Module with cairn-mail

## Summary

Replace Cairn's current GTK/GNOME-based Personal Information Management (PIM) module with **cairn-mail**, a purpose-built AI-powered email management system. This is a clean replacement—no legacy fallback option.
...
cairn-mail has been in exclusive production use on the Cairn maintainer's system (`edge` host). The integration pattern is proven and stable.
...
3. **Contacts** - Leave alone; will be pursued in cairn-mail separately
4. **`pim.enable`** = enable cairn-mail directly (no `pim.legacy`)
...
|-----------|---------|----------|
| **Email** | Geary/Evolution (GTK apps) | cairn-mail (web UI + systemd) |
...
| **Contacts** | GNOME Contacts | Cloud provider UI / future cairn-mail |
...
**Challenge**: Users may have multiple Cairn hosts on their Tailnet:
- **Server host** (e.g., `edge`): Runs cairn-mail backend, Ollama, SQLite database
...
```nix
# edge (SERVER - runs cairn-mail service)
pim = {
  enable = true;
...
|--------|-------------------|-------------------|
| cairn-mail service | ✅ Runs | ❌ Not installed |
...
**Challenge**: The cairn-mail URL is dynamic, depending on:
...
- Icon: Bundled with cairn (mail icon in `home/resources/pwa-icons/`)
...
```nix
{
  ...
  message = ''
    ...
    cairn-mail server requires Ollama for email classification.
...
{
  nixpkgs.overlays = [ inputs.cairn-mail.overlays.default ];
...
# home/pim/default.nix - New module
{
  ...
  xdg.desktopEntries.cairn-mail = lib.mkIf cfg.pwa.enable { ... };
}
```
...
3. **Contacts**: Use cloud provider web UI or await cairn-mail contacts feature
...
| Risk | Mitigation |
|------|------------|
| cairn-mail is newer | Proven in production use; calendar/contacts via other means |
...
- cairn-mail repo: `~/Projects/cairn-mail`
