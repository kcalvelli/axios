# Implementation Tasks: PIM cairn-mail Migration

## Phase 1: Flake Integration

- [ ] **1.1** Add cairn-mail flake input to `flake.nix`
  ```nix
  cairn-mail = {
...
- [ ] **4.1** Rewrite `modules/pim/default.nix` with role-based architecture:
  ...
  in
  {
    ...
    config = lib.mkIf cfg.enable {
      ...
      # Server role: import cairn-mail overlay and service
...
- [ ] **5.1** Create `home/pim/default.nix` with role-based PWA URL generation:
  ...
  in
  {
    # Only import cairn-mail home module for server role
    imports = lib.optional (isServer && inputs ? cairn-mail)
    ...
    config = lib.mkIf (pimCfg.enable or false) {
      ...
      xdg.desktopEntries.cairn-mail = lib.mkIf (pimCfg.pwa.enable or false) {
        name = "Cairn AI Mail";
      ...
      # Install PWA icon
      home.file.".local/share/icons/hicolor/128x128/apps/cairn-mail.png" =
...
- [ ] **6.2** Move `specs/pim/spec.md` to main specs directory:
  ```bash
  mv openspec/changes/pim-cairn-mail/specs/pim openspec/specs/
  ```
...
- [ ] **7.6** Test client role:
  ...
  - Verify NO cairn-mail service is installed
...
- [ ] **8.1** Archive change directory:
  ```bash
  mv openspec/changes/pim-cairn-mail openspec/changes/archive/
  ```

- [ ] **8.2** Create commit with comprehensive message documenting:
  - PIM module replacement with cairn-mail
