...
When OpenAI tooling and MCP integration are both enabled, cairn SHALL generate Codex MCP configuration declaratively and point Codex at the local `mcp-gateway` HTTP transport instead of requiring manual `codex mcp add` setup.
...
- **THEN** cairn generates `~/.codex/config.toml`
...
- **WHEN** a user leaves `services.ai.openai.enable = false`
- **THEN** cairn does not generate Codex MCP configuration
- **AND** existing Claude and Gemini behavior remains unchanged
