...
### Design Decision: Keep lazy.nvim Approach

After review, the lazy.nvim runtime download approach is **intentionally retained**. Rationale:
...
| DMS theming | Current approach integrates well with dankshell colorscheme |
...
- NOT migrating to `programs.neovim.plugins` (adds complexity for marginal purity benefit)
- NOT removing lazy.nvim (would break user customization workflow)
- NOT using Nix-managed treesitter parsers (`:TSInstall` is fine)
