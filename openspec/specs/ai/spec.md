...
- **Goose Ecosystem**:
    - `goose-cli`: Block's open-source AI agent (bleeding edge via llm-agents.nix).
    - Supports any LLM provider including local Ollama.
    - Native MCP integration - all axios MCP servers auto-configured.
    - Enable/disable: `services.ai.goose.enable` (default: true).
- **Workflow Tools**:
